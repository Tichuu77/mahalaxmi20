...
const amenities = [
  ...
  { emoji: "🎮",    title: "Gaming Zone",        description: "Indoor games and entertainment for all age groups.",           category: "entertainment" },
  { emoji: "🧘",    title: "Yoga Studio",        description: "Dedicated wellness spaces for mind and body balance.",          category: "wellness"      },
  { emoji: "🚗",    title: "Covered Parking",    description: "Secure multi-level parking for every resident.",               category: "facilities"    },
  { emoji: "🎪",    title: "Banquet Hall",       description: "Premium event spaces for celebrations and gatherings.",        category: "entertainment" },
  { emoji: "👶",    title: "Kids Play Area",     description: "Safe, colourful playground designed for children.",            category: "entertainment" },
]
...
const AmenityCard = memo(({
  ...
  return (
    <article
      className={`amenity-card stagger-item${visible ? " on" : ""} s${index}`}
      aria-label={amenity.title}
    >
      <div className="amenity-card__corner" aria-hidden="true" />
      <div className="amenity-card__icon-wrap">
        {Icon
          ? <Icon size={19} className="amenity-card__icon" aria-hidden="true" />
          : <span role="img" aria-label={amenity.title}>{(amenity as any).emoji}</span>
        }
      </div>
      <div>
