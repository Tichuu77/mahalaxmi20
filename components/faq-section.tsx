...
const faqs = [
  ...
  },
  {
    ...
    category: "pricing",
    answer:
      "Our plots start from ₹22 Lakh. Pricing varies by project, size, and location. Plots near MIHAN and Samruddhi Mahamarg are priced higher due to connectivity and appreciation potential. Contact us for a current price list for any specific project.",
  },
  {
    id: 6,
...
const FaqItem = memo(({
  ...
  <div
    className={`faq-item faq-item--${isOpen ? "open" : "closed"} stagger-item${visible ? " on" : ""} s${index}`}
  >
    <h3>
      <button
      ...
      >
        <div className="faq-item__left">
          <span
            className={`faq-item__num faq-item__num--${isOpen ? "open" : "closed"}`}
            aria-hidden="true"
          >
...
export default function FaqSection() {
  ...
  const filtered = activeChip === "all"
    ? faqs
    : faqs.filter(f => f.category === activeChip)
      ...
      <div className="section-inner">
        <div className={`rv ${vis ? "on" : ""} d0 faq__header`}>
          <div>
            ...
            <h2 id="faq-heading" className="section-heading">
              Frequently <em>Asked</em><br />
              <span>Questions</span>
            </h2>
          </div>

          <div className="faq__chips" role="group" aria-label="Filter questions by category">
            {CHIPS.map(chip => (
              <button
                key={chip}
        ...
        <div className="faq__layout">
          ...
          <div className="faq__accordion">
            ...
            <div className="faq__hint" role="complementary">
              <MessageCircle size={15} className="faq__hint-icon" aria-hidden="true" />
              <p className="faq__hint-text">
                Still have questions?{" "}
                <strong>Chat with us on WhatsApp</strong> — we respond within
                minutes.
              </p>
            </div>
          </div>
          ...
          {/* Sidebar */}
          <aside className="faq__sidebar" aria-label="Why choose Mahalaxmi Infra">
